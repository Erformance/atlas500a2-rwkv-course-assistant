...
| `02_补实验执行协议.md` | 外部合作方给定的补实验协议（P0–P6），本仓库的实验按它执行 |
...
**同一轮 P1（扩展校准：32 段分离语料 × 状态年龄 0/64/256 = 96 组）**把校准数据从 16 组扩到 96 组后，
用 `_p1_q` 后缀重建了 32 层 + 输出头，在独立测试分割上的对照：

| 评估文本 | 臂 | 全序列 KL | top-1 一致 | decode |
| --- | --- | --- | --- | --- |
| 独立测试分割 `p1test` | 旧校准 `_cp_q` | 1.0301 | 50.0% | 84.14 ms/token（11.88 tok/s） |
| 独立测试分割 `p1test` | 新校准 `_p1_q` | **0.8563** | **57.8%** | 83.54 ms/token（11.97 tok/s） |
| 旧文本 `pilot` | 旧校准 `_cp_q` | 0.9452 | 67.2% | — |
| 旧文本 `pilot` | 新校准 `_p1_q` | **0.6857** | **71.9%** | — |

结论：扩展校准让 KL 降 17%~28%、top-1 提高 4.7~7.8 个百分点、状态漂移降 29%，速度不变。
同时要注意**评估口径**：表里 67.2% 那一行用的是"校准也用过的文本"，独立测试分割上旧校准只有 50.0%
——对外汇报应当引用独立分割的数字。语料、采集、重建脚本见 `p1_corpus.py`、
`capture_calib_p1.py`、`pack_calib_p1.py`、`build_p1_layers.sh`、`p1_chain.sh`。
